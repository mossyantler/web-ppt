...
문단(`data-el="text"`) 안에는 인라인 수식이 산다. 실측 — W31 의 수식 53 개 중 **18 개**가 문단 안이고, 저작 리프 87 개 중 8 개가 그런 문단이다.

정화기가 살리는 속성은 `class`·`href` 뿐이므로, 문단을 통째로 저장하면 그 안의 `data-el`·`data-node-id`·`data-tex` 가 전부 떨어져 나가고 **수식이 조용히 사라진다.** 화면에 보이는 것은 KaTeX 가 그린 결과라서 그대로 되돌려 보낼 수도 없다.

그래서 `setContent` 는 자리표를 받는다.

```
화면이 보내는 것   ...운동량방정식에 <span data-node-id="n1c"></span> 항으로...
파일에 써지는 것   ...운동량방정식에 <span data-el="equation" data-node-id="n1c" data-tex="\tau_s"></span> 항으로...
```

`content-commands.js` 가 대상 리프 안의 이름표 붙은 노드를 `id → 원문 바이트` 로 모아 정화기에 넘기고, 정화기는 자리표를 만나면 **그 바이트를 그대로** 낸다. 자리표 **안**에 무엇이 들어 있든 원문이 이긴다 — 화면이 실수로 렌더 결과를 되돌려 보내도 파일에는 못 들어간다.

- 자리표가 오지 않은 자식은 **지워진다** (사용자가 문단에서 수식을 지운 경우). 지울 방법이 없으면 편집기가 아니다
- 이 리프의 자식이 아닌 id → 422. 통과시키면 문서 안에 같은 id 가 둘이 되거나 남의 노드가 복제된다
- 같은 자리표가 두 번 → 422 (id 유일성)

수식 **자체**의 편집은 `setTex` 이고 전용 UI 는 M3-8 이다.
...
| 입력 | `normalize-inline.js` | 스크립트·이벤트 핸들러·`javascript:` 는 **거부**. 나머지는 언랩·제거로 통과. 이 리프의 자식이 아닌 `data-node-id` 와 중복 자리표도 거부 |
