...
- **칸 나눈 격자의 좌우 여백 (2026-08-24):** 세로 구분선으로 칸을 나눌 때, 여백을
  `> * + *` 에만 주면 **첫 칸만 왼쪽에 여백이 없어 경계에 붙는다.** `gap` 은 칸 사이에만
  들어가고 바깥쪽에는 안 붙기 때문이다. 판단 기준은 **첫 칸 왼쪽에 선이 있는가** 하나다.
  - 격자가 다른 칸 **안에** 있으면(예: 5:7 분할의 오른쪽 칸 안) 첫 칸 왼쪽에 부모의
    세로선이 있다 → **모든 칸에 같은 왼쪽 여백**을 준다. 안 그러면 첫 칸만 선에 붙는다.
  - 격자가 본문 정렬선에서 시작하면 첫 칸 왼쪽에는 선이 없다 → 첫 칸은 여백 없이 두어
    **제목·문단과 같은 세로선**에 세운다. 여백을 주면 그 정렬선이 깨진다.
  - 실제 적용: `gauge-legend`·`review-scope`·`stage-grid`·`term-row`·`flow-chain` 은 전자,
    `split-57`·`result-grid`·`pair-grid`·`equation-grid` 는 후자.
...
- **SVG 아이콘 — 편집기 UI 한정으로 Lucide 도입 (2026-08-24):**
  **슬라이드에는 여전히 아이콘을 쓰지 않습니다.** 위 방침은 그대로입니다. 도입한 곳은
  편집기 껍데기(도구 모음·슬라이드 목록·선택 버튼바)뿐이고, 거기서는 `↶ ⧉ 🗑` 같은
  유니코드 글리프가 글꼴마다 다른 모양·크기로 나오던 문제를 풀기 위한 것입니다.
  - 세트: [Lucide](https://lucide.dev) (ISC). 24px 그리드, 2px 스트로크, 라운드 캡.
  - 방식: **CDN 을 쓰지 않습니다.** 편집기는 저장소 안의 자산만 내주므로(KaTeX 와 같은
    규칙), 쓰는 것만 골라 `server/ui/icons.js` 로 뽑아 둡니다. 2035 개 중 20 개입니다.
  - 추가하려면: `tools/gen-icons.mjs` 의 `WANT` 에 이름을 넣고 `npm run icons`.
  - 크기·굵기·색은 CSS(`.ico`)가 정합니다 — SVG 파일에 박아 두면 버튼마다 다른 크기를
    줄 수 없습니다.
